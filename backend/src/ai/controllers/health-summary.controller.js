...
/**
 * GET /health-summary/me — Patient retrieves their own summary.
 */
const getMySummary = async (req, res, next) => {
  try {
    const dto = await healthSummaryService.getPatientSummary(req.user.id, req.user.role, req.user.id);
    return res.status(200).json(
      new ApiResponse(200, dto, 'Your health summary retrieved successfully.')
    );
  } catch (error) {
    return next(error);
  }
