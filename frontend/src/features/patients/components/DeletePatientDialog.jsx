...
export const DeletePatientDialog = ({ isOpen, onClose, onConfirm, patientName = 'this patient' }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-popover border border-border/60 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4 relative">
        ...
        <div className="flex items-center space-x-3 text-rose-500">
          ...
          <div>
            <h3 className="text-base font-bold text-foreground">Archive Patient Record</h3>
            <p className="text-xs text-muted-foreground">Action confirmation</p>
          </div>
        </div>

        <p className="text-xs text-muted-foreground leading-relaxed">
          Are you sure you want to deactivate or archive the EHR profile for <strong className="text-foreground">{patientName}</strong>? Active appointments and vital alerts will remain stored in historical audit logs.
        </p>
