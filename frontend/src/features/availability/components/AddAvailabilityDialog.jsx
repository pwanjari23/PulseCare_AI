    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          ...
          >
            <button onClick={onClose} className="absolute top-4 right-4 p-1 rounded-lg text-muted-foreground hover:text-foreground" aria-label="Close dialog">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 id="add-avail-title" className="text-base font-bold text-foreground">Add Availability Block</h3>
                <p className="text-xs text-muted-foreground">Set working hours for a day of the week</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="flex items-center justify-end space-x-3 pt-2 border-t border-border/40">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-primary text-primary-foreground text-xs font-bold rounded-xl hover:bg-primary/90 shadow-md shadow-primary/20 transition-all disabled:opacity-60 flex items-center space-x-1.5"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>{createMutation.isPending ? 'Adding...' : 'Add Block'}</span>
                </button>
              </div>
            </form>
          </motion.div>
